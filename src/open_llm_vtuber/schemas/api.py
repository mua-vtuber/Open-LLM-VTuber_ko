...
class ErrorResponse(BaseModel):
    ...
    model_config = {
        "json_schema_extra": {"example": {"error": "요청 처리 중 오류가 발생했습니다"}}
    }
...
class SuccessResponse(BaseModel):
    ...
    model_config = {
        "json_schema_extra": {
            "example": {"success": True, "message": "작업이 성공적으로 완료되었습니다"}
        }
    }
...
class ModelInfo(BaseModel):
    ...
    name: str = Field(
        ..., description="모델 이름", json_schema_extra={"example": "mao_pro"}
    )
...
class ModelListResponse(BaseModel):
    ...
    type: str = Field(default="live2d-models/info", description="응답 타입")
    count: int = Field(..., description="모델 수", json_schema_extra={"example": 3})
    characters: list[ModelInfo] = Field(..., description="모델 정보 목록")
...
class ExternalFolderInfo(BaseModel):
    ...
    path: str = Field(
        ..., description="폴더 경로", json_schema_extra={"example": "D:/MyModels"}
    )
...
class ExternalFoldersResponse(BaseModel):
    ...
    folders: list[ExternalFolderInfo] = Field(..., description="등록된 외부 폴더 목록")
...
class AddFolderRequest(BaseModel):
    ...
    model_config = {"json_schema_extra": {"example": {"path": "D:/MyModels"}}}
...
class AddFolderResponse(BaseModel):
    ...
    path: str = Field(..., description="추가된 폴더 경로")
    mount_path: str = Field(..., description="마운트 경로")
    message: Optional[str] = Field(
        None, description="추가 메시지 (이미 등록된 경우 등)"
    )
...
class RemoveFolderResponse(BaseModel):
    ...
    success: bool = Field(..., description="제거 성공 여부")
    path: str = Field(..., description="제거된 폴더 경로")
    message: Optional[str] = Field(None, description="추가 메시지")
...
class QueueStatus(BaseModel):
    ...
    pending: int = Field(
        ..., description="대기 중인 메시지 수", json_schema_extra={"example": 5}
    )
    ...
    max_size: int = Field(
        ..., description="최대 큐 크기", json_schema_extra={"example": 100}
    )
    total_received: int = Field(
        ..., description="총 수신 메시지 수", json_schema_extra={"example": 150}
    )
    total_processed: int = Field(
        ..., description="총 처리 완료 메시지 수", json_schema_extra={"example": 145}
    )
    total_dropped: int = Field(
        ..., description="드롭된 메시지 수", json_schema_extra={"example": 0}
    )
    running: bool = Field(
        ..., description="큐 매니저 실행 상태", json_schema_extra={"example": True}
    )
    avg_processing_time: Optional[float] = Field(
        None, description="평균 처리 시간 (초)", json_schema_extra={"example": 2.5}
    )
    processing_rate: Optional[float] = Field(
        None, description="처리 속도 (msg/s)", json_schema_extra={"example": 0.4}
    )
...
class QueueHistoryItem(BaseModel):
    ...
    pending: int = Field(..., description="대기 중인 메시지 수")
    processing: int = Field(..., description="처리 중인 메시지 수")
    processing_rate: Optional[float] = Field(None, description="처리 속도")
...
class QueueHistoryResponse(BaseModel):
    ...
    minutes: int = Field(
        ..., description="조회 기간 (분)", json_schema_extra={"example": 5}
    )
    data_points: int = Field(
        ..., description="데이터 포인트 수", json_schema_extra={"example": 30}
    )
    history: list[QueueHistoryItem] = Field(..., description="히스토리 데이터")
...
class PriorityRules(BaseModel):
    ...
    wait_time: float = Field(
        ..., description="대기 시간 (0~30초)", json_schema_extra={"example": 5.0}
    )
    allow_interruption: bool = Field(
        ..., description="중단 허용 여부", json_schema_extra={"example": True}
    )
    superchat_always_priority: bool = Field(
        ..., description="슈퍼챗 항상 우선", json_schema_extra={"example": True}
    )
...
class PriorityRulesUpdateResponse(BaseModel):
    ...
    priority_rules: PriorityRules = Field(..., description="업데이트된 우선순위 규칙")
...
class LiveConfigResponse(BaseModel):
    ...
    chat_monitor: dict[str, Any] = Field(..., description="채팅 모니터 설정")
        ...
        "json_schema_extra": {
            "example": {
                "chat_monitor": {
                    ...
                    "max_retries": 10,
                    "retry_interval": 60,
                    "youtube": {"enabled": False, "api_key": "", "channel_id": ""},
                    "chzzk": {
                        "enabled": False,
                        "channel_id": "",
                        "client_id": "",
...
class LiveConfigUpdateResponse(BaseModel):
    ...
    success: bool = Field(..., description="업데이트 성공 여부")
    live_config: dict[str, Any] = Field(..., description="업데이트된 라이브 설정")
...
class TranscriptionResponse(BaseModel):
    ...
    model_config = {
        "json_schema_extra": {"example": {"text": "안녕하세요, 오늘 날씨가 좋네요."}}
    }
...
class TTSMessage(BaseModel):
    ...
    model_config = {
        "json_schema_extra": {"example": {"text": "안녕하세요. 반갑습니다."}}
    }
...
class TTSResponse(BaseModel):
    ...
    message: Optional[str] = Field(None, description="오류 메시지 (오류 발생 시)")
...
class LanguageInfo(BaseModel):
    ...
    code: str = Field(
        ..., description="언어 코드 (ISO 639-1)", json_schema_extra={"example": "ko"}
    )
    label: str = Field(
        ..., description="언어 표시명 (원어)", json_schema_extra={"example": "한국어"}
    )

    model_config = {"json_schema_extra": {"example": {"code": "ko", "label": "한국어"}}}


class LanguagesResponse(BaseModel):
    """사용 가능한 언어 목록 응답 스키마."""

    type: str = Field(default="available_languages", description="응답 타입")
    count: int = Field(..., description="언어 수", json_schema_extra={"example": 3})
    languages: list[LanguageInfo] = Field(..., description="언어 목록")
